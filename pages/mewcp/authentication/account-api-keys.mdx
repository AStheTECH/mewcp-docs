...
```bash
curl https://gateway.mewcp.com/audience \
...
```typescript
const res = await fetch("https://gateway.mewcp.com/audience", {
...
import httpx
resp = httpx.get(
    "https://gateway.mewcp.com/audience",
...
| List your audience | `GET /audience` |
| Get an audience member's credentials | `GET /audience/{externalUserId}` |
| Revoke all credentials for a user | `DELETE /audience/{externalUserId}` |
| Revoke a single credential | `DELETE /audience/{externalUserId}/credentials/{credentialId}` |
